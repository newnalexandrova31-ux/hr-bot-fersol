...
def clean_markdown_formatting(text):
    ...
    # Standardize ID terminology (ID номер -> ID-номер)
    text = re.sub(r'ID\s+номер', 'ID-номер', text, flags=re.IGNORECASE)
...
4. ОБРАЩЕНИЕ: Всегда обращайся к сотруднику на "Вы" (с большой буквы или вежливо). Никогда не используй "ты".
5. ЕСЛИ ИНФОРМАЦИИ НЕТ В КОНТЕКСТЕ, ОТВЕТЬ СТРОГО ЭТОЙ ФРАЗОЙ: «К сожалению, у меня нет информации по этому вопросу. Пожалуйста, обратитесь к HR-менеджеру для уточнения.»
6. ОФОРМЛЕНИЕ (СТРОГО):
   ...
   - 🔎 ПРОВЕРЯЙ ОРФОГРАФИЮ: Недопустимы опечатки (например, пиши "отпускных", а не "отпуксных").
   - 🆔 ТЕРМИНОЛОГИЯ: Пиши "ID-номер" (через дефис) или "номер ID", но не "ID номер".
